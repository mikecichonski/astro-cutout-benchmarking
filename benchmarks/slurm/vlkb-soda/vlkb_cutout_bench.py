#!/usr/bin/env python3
"""Benchmark harness for vlkb cutouts on Dug via sbatch + seff."""
...
def derive_centered_filter(header: Dict[str, Any], target_mib: int) -> Dict[str, Any]:
    ...
    estimated_bytes = bytes_per_pixel * product([end - start + 1 for start, end in bounds])
...
def derive_fraction_filter(
    ...
    *,
    ...
    estimated_bytes = int(round(bytes_per_pixel * product(dims) * actual_fraction))
    pixel_filter = "[" + ",".join(f"{start}:{end}" for start, end in bounds) + "]"
...
def build_scan_context(
    ...
    base_row: Dict[str, Any] = {
        ...
        "rows_per_chunk": (manifest_row or {}).get("rows_per_chunk", "") or benchmark.get("rows_per_chunk", ""),
...
def build_benchmark_command(
    *,
    ...
    command.append(str(rows_per_chunk))
    return command
...
def run_cmd(command: Sequence[str], cwd: Optional[pathlib.Path] = None) -> subprocess.CompletedProcess:
...
def add_submit_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=[1],
        help="Rows per C++ imcopy/imcopydav read/write chunk. Values are crossed with sources and workloads.",
    )
    parser.add_argument(
        "--worker-strategy",
...
def add_scan_errors_parser(subparsers: argparse._SubParsersAction) -> None:
...
def add_stage_start_matrix_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        "stage-start-matrix",
...
def add_submit_start_matrix_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        "submit-start-matrix",
...
def add_collect_start_matrix_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
...
def add_run_location_matrix_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        "run-location-matrix",
    ...
    parser.add_argument("--job-name-prefix", default="vlkb-locxy")
    parser.add_argument(
        "--env-script",
...
def add_run_storage_chunk_matrix_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        "run-storage-chunk-matrix",
        help="Run a compact vlkb-soda follow-up matrix crossing local/remote source access with rows-per-chunk.",
    )
    parser.add_argument("--binary", required=True, help="Path to the Davix-enabled vlkb binary on Dug.")
    parser.add_argument("--local-source", required=True, help="Local FITS path to the full original cube.")
    parser.add_argument("--remote-source", help="HTTP(S) FITS URL for the same cube in object storage.")
    parser.add_argument("--extnum", type=int, default=0)
    parser.add_argument("--cpus", type=int, default=64, help="Single --cpus-per-task value for every job.")
    parser.add_argument("--mem", default="8G", help="SBATCH --mem value for every job.")
    parser.add_argument(
        "--rows-per-chunk",
        nargs="+",
        type=int,
        default=[1, 10, 100, 1000],
        help="Rows per imcopy/imcopydav chunk for both sources unless a source-specific override is provided.",
    )
    parser.add_argument(
        "--local-rows-per-chunk",
        nargs="+",
        type=int,
        help="Rows per imcopy chunk for local-source rows. Overrides --rows-per-chunk for local rows.",
    )
    parser.add_argument(
        "--remote-rows-per-chunk",
        nargs="+",
        type=int,
        help="Rows per imcopydav chunk for remote-source rows. Overrides --rows-per-chunk for remote rows.",
    )
    parser.add_argument(
        "--cut-fractions",
        nargs="+",
        ...
        help="Cutout data fractions to derive from the original cube.",
    )
    parser.add_argument(
        "--dimensions",
        nargs="+",
        choices=("xy", "xyz"),
        default=["xy"],
        help="Cutout dimension modes. Keep the default xy for the smallest follow-up matrix.",
    )
    parser.add_argument(
        "--locations",
        nargs="+",
    ...
    parser.add_argument("--session-dir", help="Existing or explicit session directory to resume/use.")
    parser.add_argument("--job-name-prefix", default="vlkb-chunk")
    parser.add_argument("--env-script", help="Shell script sourced by the sbatch wrapper before each benchmark.")
    parser.add_argument("--time", help="SBATCH --time value, e.g. 08:00:00.")
    parser.add_argument("--partition", help="SBATCH --partition value.")
    parser.add_argument("--account", help="SBATCH --account value.")
    parser.add_argument("--qos", help="SBATCH --qos value.")
    ...
    parser.add_argument(
        "--force-rerun",
        action="store_true",
        help="Replace existing row directories and manifest rows.",
    )
    parser.add_argument(
        "--rerun-failed-only",
        action="store_true",
        help="When resuming a session, keep successful rows and replace rows with failed or incomplete artifacts.",
    )
    parser.add_argument(
        "--skip-running",
        action="store_true",
        help="When resuming a session, do not resubmit rows whose prior Slurm jobs are still active.",
    )
    parser.add_argument("--dry-run", action="store_true", help="Print sbatch commands without submitting.")
    parser.set_defaults(func=cmd_run_storage_chunk_matrix)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    add_run_storage_chunk_matrix_parser(subparsers)
...
def format_chunk_combo_name(
    source_name: str,
    cpus: int,
    workers: int,
    rows_per_chunk: int,
    target_mib: Optional[int],
    pixel_filter: Optional[str],
) -> str:
    return f"{format_combo_name(source_name, cpus, workers, target_mib, pixel_filter)}_r{rows_per_chunk}"
...
def cmd_submit(args: argparse.Namespace) -> int:
    ...
    if any(rows < 1 for rows in args.rows_per_chunk):
        raise SystemExit("--rows-per-chunk values must all be >= 1")

    script_path = pathlib.Path(__file__).resolve()
    default_job_script = script_path.with_name("vlkb_benchmark_job.sbatch")
    ...
    for source_name, mode, source in source_specs(args):
        for cpus in args.cpus:
            for workers in worker_counts(args.worker_strategy, cpus):
                for rows_per_chunk in args.rows_per_chunk:
                    for target_mib, pixel_filter in workloads:
                        combo_name = format_chunk_combo_name(
                            source_name,
                            cpus,
                            workers,
                            rows_per_chunk,
...
def slurm_job_state(job_id: str) -> str:
    ...
    if not job_id:
        return ""
    completed = run_cmd(["squeue", "-h", "-j", job_id, "-o", "%T"])
    if completed.returncode != 0:
        return ""
...
def active_slurm_job_for_row(output_dir: pathlib.Path, manifest_row: Optional[Dict[str, str]]) -> Tuple[str, str]:
    candidates: List[str] = []
    if manifest_row is not None:
        candidates.append(str(manifest_row.get("job_id", "") or ""))
    candidates.append(infer_job_id_from_output_dir(output_dir))
...
def build_location_matrix_manifest_row(
    *,
    ...
    return {
        ...
        "requested_memory": row["requested_memory"],
        "parallel_workers": 1,
        "rows_per_chunk": row.get("rows_per_chunk", 1),
        "input_size_bytes": row["input_size_bytes"],
        "input_size_label": row["input_size_label"],
        "input_size_gib": row.get("input_size_gib", int(round(row["input_size_bytes"] / (1024 ** 3)))),
        "file_name": pathlib.Path(row["input_path"]).name,
...
def build_storage_chunk_manifest_row(
    *,
    job_id: str,
    job_name: str,
    row: Dict[str, Any],
    ...
    return {
        ...
        "requested_memory": row["requested_memory"],
        "parallel_workers": 1,
        "rows_per_chunk": row["rows_per_chunk"],
        "input_size_bytes": row["input_size_bytes"],
        "input_size_label": row["input_size_label"],
        "input_size_gib": row["input_size_gib"],
        ...
        "cutout_size_label": row["cutout_size_label"],
        "cutout_size_percent": int(round(row["cut_fraction"] * 100)),
        "cutout_dimensions": row["cutout_dimensions"],
        "cutout_location": row["cutout_location"],
        "notes": row.get("notes", ""),
        "output_dir": str(output_dir),
        "result_json": str(output_dir / "result.json"),
...
def storage_chunk_combo_name(row_index: int, row_width: int, row: Dict[str, Any]) -> str:
    dimensions = "xyz" if row["cutout_dimensions"] == "x,y,z" else "xy"
    return (
        f"{row['source_name']}_row{row_index:0{row_width}d}_"
        f"c{row['cpus']}_r{row['rows_per_chunk']}_"
        f"{fraction_tag(row['cut_fraction'])}_{dimensions}_"
        f"{slugify(row['cutout_location'])}"
    )


def chunk_values_for_source(args: argparse.Namespace, source_name: str) -> List[int]:
    if source_name == "local" and args.local_rows_per_chunk:
        return args.local_rows_per_chunk
    if source_name == "remote" and args.remote_rows_per_chunk:
        return args.remote_rows_per_chunk
    return args.rows_per_chunk


def storage_chunk_matrix_rows(args: argparse.Namespace) -> List[Dict[str, Any]]:
    if args.cpus < 1:
        raise SystemExit("--cpus must be >= 1")
    if any(rows < 1 for rows in args.rows_per_chunk):
        raise SystemExit("--rows-per-chunk values must all be >= 1")
    if args.local_rows_per_chunk and any(rows < 1 for rows in args.local_rows_per_chunk):
        raise SystemExit("--local-rows-per-chunk values must all be >= 1")
    if args.remote_rows_per_chunk and any(rows < 1 for rows in args.remote_rows_per_chunk):
        raise SystemExit("--remote-rows-per-chunk values must all be >= 1")
    if any(not (0.0 < value <= 1.0) for value in args.cut_fractions):
        raise SystemExit("--cut-fractions values must all be within (0, 1].")

    local_source = pathlib.Path(args.local_source).resolve()
    if not local_source.exists():
        raise SystemExit(f"Local source FITS file not found: {local_source}")

    header = parse_fits_header(str(local_source))
    size_bytes = local_source.stat().st_size
    size_label = human_gib_approx(size_bytes)
    input_size_gib = int(round(size_bytes / (1024 ** 3)))

    source_specs_for_rows: List[Tuple[str, str, str]] = [
        ("local", "imcopy", str(local_source)),
    ]
    if args.remote_source:
        source_specs_for_rows.append(("remote", "imcopydav", args.remote_source))

    rows: List[Dict[str, Any]] = []
    for source_name, mode, source in source_specs_for_rows:
        source_chunks = chunk_values_for_source(args, source_name)
        for location in args.locations:
            location_key = normalize_location_key(location)
            if location_key not in {"start", "middle", "end"}:
                raise SystemExit(f"Unsupported location: {location}")
            for dimensions in args.dimensions:
                for cut_fraction in args.cut_fractions:
                    cutout = derive_fraction_filter(
                        header,
                        cut_fraction,
                        axes_mode=dimensions,
                        anchor=location_anchor_for_filter(location_key),
                    )
                    cutout_dimensions = "x,y,z" if dimensions == "xyz" else "x,y"
                    for rows_per_chunk in source_chunks:
                        rows.append(
                            {
                                "tool": tool_label("vlkb", mode),
                                "source_name": source_name,
                                "mode": mode,
                                "input_path": source,
                                "extnum": args.extnum,
                                "cpus": args.cpus,
                                "requested_memory": args.mem,
                                "rows_per_chunk": rows_per_chunk,
                                "input_size_bytes": size_bytes,
                                "input_size_label": size_label,
                                "input_size_gib": input_size_gib,
                                "file_name": source_basename(source),
                                "cut_fraction": cut_fraction,
                                "cutout_size_bytes": cutout["estimated_bytes"],
                                "cutout_size_label": cutout_size_label(
                                    cutout["estimated_bytes"],
                                ...
                                "cutout_dimensions": cutout_dimensions,
                                "cutout_location": location_title(location_key),
                                "notes": "original cube only",
                            }
                        )
    return rows


def cmd_run_storage_chunk_matrix(args: argparse.Namespace) -> int:
    script_path = pathlib.Path(__file__).resolve()
    default_job_script = script_path.with_name("vlkb_benchmark_job.sbatch")
    job_script = pathlib.Path(args.job_script).resolve() if args.job_script else default_job_script
    if not job_script.exists():
    ...
    if args.session_dir:
        ...
        session_dir = ensure_dir(
            results_root / datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S_storage_chunk")
        )
    manifest_path = session_dir / "manifest.tsv"
    session_meta = {
        "created_at_utc": utc_now(),
        "last_run_at_utc": utc_now(),
        "submit_args": {key: value for key, value in vars(args).items() if key != "func"},
        "job_script": str(job_script),
        "storage_strategy": "original cube only; cross local/remote source access with rows-per-chunk",
    }
    write_json(session_dir / "session.json", session_meta)

    rows = storage_chunk_matrix_rows(args)
    row_width = max(2, len(str(len(rows))))
    ...
    for row_index, row in enumerate(rows, start=1):
        combo_name = storage_chunk_combo_name(row_index, row_width, row)
        ...
        if args.skip_running:
            active_job_id, active_job_state = active_slurm_job_for_row(output_dir, existing_row)
            if active_job_id:
                wait_job_ids.append(active_job_id)
                print(f"skip_running\t{combo_name}\t{active_job_id}\t{active_job_state}")
                continue
        ...
        if row_has_execution_artifacts(output_dir) and not (output_dir / "result.json").exists():
            if args.force_rerun or args.rerun_failed_only:
                ...
                raise SystemExit(
                    ...
                    f"Wait for it to finish or use --rerun-failed-only/--force-rerun.{prior_job_text}"
                )
        ...
        if args.dry_run:
            print(shell_join(command))
            manifest_row = build_storage_chunk_manifest_row(
                job_id="",
                job_name=job_name,
                row=row,
                output_dir=output_dir,
        ...
        if completed.returncode != 0:
            sys.stderr.write(completed.stderr)
            raise SystemExit(f"sbatch failed for {combo_name} with exit code {completed.returncode}")

        job_id = parse_sbatch_job_id(completed.stdout)
        wait_job_ids.append(job_id)
        manifest_row = build_storage_chunk_manifest_row(
            job_id=job_id,
            job_name=job_name,
            row=row,
    ...
    if args.dry_run:
        print(f"\nDry run complete. Session directory: {session_dir}")
        print(f"Manifest: {manifest_path}")
        return 0

    wait_for_slurm_jobs(wait_job_ids)
    ...
    summary_csv = session_dir / "storage_chunk_summary.csv"
    summary_md = session_dir / "storage_chunk_summary.md"
    write_start_matrix_summary(
        manifest_path,
        enriched_rows,
        summary_csv=summary_csv,
        summary_md=summary_md,
        title="VLKB-SODA Storage And Chunk Matrix Summary",
    )
    print(f"\nManifest: {manifest_path}")
    print(f"Enriched: {enriched_tsv}")
        ...
        table_rows.append(
            {
                ...
                "Rows/Chunk": row.get("rows_per_chunk", "") or row.get("runner_benchmark_rows_per_chunk", ""),
